...
try:
    ...
    sys.exit("SDV library not found. Please install it with: pip install sdv")
...
def load_dataset(dataset_name):
    ...
    try:
        ...
        raise FileNotFoundError(f"Dataset file not found at {config['path']}. Please check the path.")
...
def load_model():
    ...
    try:
        return joblib.load("ctgan_1.20.0.pkl")
    except FileNotFoundError:
...
def generate_synthetic_data(model, num_rows):
    ...
    try:
        ...
        raise AttributeError("The loaded model doesn't have a sample() method. Are you sure this is a trained SDV model?")
    except Exception as e:
