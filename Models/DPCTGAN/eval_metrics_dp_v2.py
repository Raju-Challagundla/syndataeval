...
def evaluate_synthetic_data(real_df, target, epsilons, synth_size=1000, dataset_name=""):
    ...
    for epsilon in epsilons:
        print(f"\n--- ε = {epsilon} ---")
        synth = Synthesizer.create("dpctgan", epsilon=epsilon, verbose=False)
        synth.fit(real_df, preprocessor_eps=1.0)
        filename = f"pkl/dpctgan/dpctgan_{dataset_name}_epsilon_{epsilon}.pkl"
...
def plot_results(results, classification, dataset_name):
    ...
    plt.title(f"DPCTGAN_Privacy-Utility Tradeoff - {dataset_name.upper()}")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
   # plt.show()
    ...
    filename = f"privacy_utility_{dataset_name}_{timestamp}.pdf"
