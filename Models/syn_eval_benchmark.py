...
def save_results(all_results):
        """Save results to CSV files with p-values"""
        fd_metrics = []
        stat_sim = []
        privacy_metrics = []
        ml_utility = []
        ...
        for result in all_results:
            ...
            privacy_metrics.append({
                ...
                # 'MIA_MSE_p': pvals.get('MSE'),
                # 'MIA_R2': attack.get('R2'),
                # 'MIA_R2_p': pvals.get('R2'),
                # 'Membership_inference_attack_AUROC': attack.get('Membership inference attack AUROC'),
                # 'Membership_inference_attack_AUROC_p': pvals.get('Membership inference attack AUROC')
            })
            
            # ML Utility with p-values
            ml_utility.append({
                'dataset': dataset,
                'model': model,
        ...
        save_metrics_with_error_bars(metrics, output_dir="Results/")
        ...
        for name, data in metrics.items():
            ...
            df.to_csv(f'Results/{name}_with_pvalues.csv', index=False)
            print(f"Saved {name}_with_pvalues.csv")
...
for dataset_name in ["adult","credit"]:
    ...
    for model_name, model_class, model_params in models_to_evaluate:
        ...
        # Store results for all synthetic trials
        ...
        if len(synth_utility_metrics) > 0:
            non_numeric_cols = pd.DataFrame(synth_utility_metrics).select_dtypes(exclude='number').columns
            if len(non_numeric_cols) > 0:
                print(f"Non-numeric utility metric fields ignored in mean(): {list(non_numeric_cols)}")
            non_numeric_attack_cols = pd.DataFrame(synth_utility_metrics).select_dtypes(exclude='number').columns
            if len(non_numeric_attack_cols) > 0:
                print(f"Non-numeric utility metric fields ignored in mean(): {list(non_numeric_attack_cols)}")
            ...
            # mean_utility = pd.DataFrame(synth_utility_metrics).mean().to_dict()
            # mean_attack = pd.DataFrame(synth_attack_metrics).mean().to_dict()
            
            # Calculate p-values by comparing single real vs distribution of synthetic
            utility_p_values = SyntheticDataMetrics.calculate_p_values(
                real_utility_list * len(synth_utility_metrics),  # Repeat real metrics
                synth_utility_metrics
            )
            attack_p_values = SyntheticDataMetrics.calculate_p_values(
                real_attack_list * len(synth_attack_metrics),  # Repeat real metrics
                synth_attack_metrics
            )
            ...
            all_results.append({
                ...
                'real_utility': real_utility,  # Store baseline for reference
                'real_attack': real_attack    # Store baseline for reference
            })
